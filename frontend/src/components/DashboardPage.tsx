...
export const DashboardPage: React.FC = () => {
	...
	const [q, setQ] = useState<Record<string,string>>({"sectorFilter":"","profitLossRange":"","datePurchased":""});
	...
	return (
		<div className='table-wrapper'>
			<div className='filters'><label style={{marginRight:8}}>Sector Filter: <input value={q.sectorFilter || ''} onChange={e => setQ(v => ({...v, sectorFilter: e.target.value}))} /></label> <label style={{marginRight:8}}>Profit/loss Range: <input value={q.profitLossRange || ''} onChange={e => setQ(v => ({...v, profitLossRange: e.target.value}))} /></label> <label style={{marginRight:8}}>Date Purchased.: <input value={q.datePurchased || ''} onChange={e => setQ(v => ({...v, datePurchased: e.target.value}))} /></label> </div>
			<table className='data-table'>
				...
				<tbody>{sorted.map((r,i) => (<tr key={i}><td>{r.stockName}</td><td>{r.sector}</td><td>{r.units}</td><td>{r.buyPrice}</td><td>{r.currentPrice}</td><td>{r.totalValue}</td><td>{r.profitLoss}</td></tr>))}</tbody>
			</table>
		</div>
	);
};
